...
from worker.audio_engine import analyze_audio
from worker.mastering import master_track
from worker.mixing import mix_stems
...
def claim_next_job():
    # Atomically claim a queued job by flipping status.
...
def handle_job(job):
    ...
    try:
        if workflow == "master" or workflow == "master-only":
